...
export function set (target: Array<any> | Object, key: any, val: any): any {
  ...
  // 收集不到对应的依赖（没有使用defineReactive定义），所以使用this.key（这个操作可以理解为
  // this._data.key）的时候由于data没有自己的闭包依赖，所以所有的模板依赖都无法收集到。
  // 自然在使用Vue.set的时候触发不了对应依赖的响应式了。
  // $data的依赖只能是watch手动的添加。
